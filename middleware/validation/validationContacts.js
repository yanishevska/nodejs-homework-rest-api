...
const validateCreate = async (req, res, next) => {
    try {
        ...
        return res.status(HttpCode.BAD_REQUEST).json({ message: `Field ${err.message.replace(/"/g, '')}` })
    }
    next()
}
...
const validateUpdate = async (req, res, next) => {
    try {
        ...
        if (type === "object.missing") {
            return res.status(HttpCode.BAD_REQUEST).json({ message: `${MESSAGES.MISSING}` })
        }
...
const validateId = async (req, res, next) => {
    if (!Types.ObjectId.isValid(req.params.id)) {
        return res.status(HttpCode.BAD_REQUEST).json({ message: `${MESSAGES.INVALID}` })
    }
    next()
}
...
const validateUpdateFavorite = async (req, res, next) => {
    try {
        ...
        if (type === "object.unknown") {
            return res.status(HttpCode.BAD_REQUEST).json({ message: `${MESSAGES.MISSING_FAVORITE}` })
        }
        return res.status(HttpCode.BAD_REQUEST).json({ message: err.message.replace(/"/g, '') })
    }
    next()
